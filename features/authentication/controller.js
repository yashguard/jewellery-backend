...
class controller {
    ...
    static addStaff = async (req,res) => {
        try {
            const {username,phone,email,role,joiningDate,empId,password,confirmPassword} = req.body;
            ...
            const doc = {
                username,phone,email,role,joiningDate,empId,password: hashPassword,
                authProvider: "local",isActive: true,isVerified: true
            };
    ...
    static register = async (req,res) => {
        try {
            const {username,password,email,phone,authProvider,connection,link,connectionType} = req.body;
            ...
            if (email === config.admin.email) {role = userRoleEnum.ADMIN;}
    ...
    static googleLogin = async (req,res) => {
        try {
            ...
            if (!user) {
                ...
                await AuthModel.updateOne({_id: newUser._id},{$set: {otp: null,otpExpiration: null,password: null,role}});
            }
    ...
    static verifyToken = async (req,res) => {
        try {
            ...
            const result = await AuthModel.findById(userId).select('username email phone authProvider url customerId role empId');
            return successResponse({
                res,
                statusCode: 200,
                data: result,
    ...
    static getCustomer = async (req,res) => {
        try {
            ...
            const result = await AuthModel.find(filter)
                .select("joiningDate customerId username customerId phone isActive url email");
            paginationData = paginationDetails({
                limit: pagination.limit,
                page: req.query.page,
                totalItems: count,
    ...
    static customerDetails = async (req,res) => {
        try {
            ...
            let filter = {};
            if (id) filter._id = new mongoose.Types.ObjectId(id);
            if (customerId) filter.customerId = {$regex: new RegExp(`^${ customerId }$`,'i')};

            const user = await AuthModel.findOne({customerId: customerId})
                .select("username email phone customerId url phone joiningDate createdAt");
            const orders = await OrderModel.find({user: user._id})
                .select("user shippingAddress billingAddress orderId items totalAmount status paymentMethod createdAt updatedAt")
                .sort({createdAt: -1});
        ...
        try {
            ...
            if (req.user.role === userRoleEnum.ADMIN) {
                ...
                result = await AuthModel.find(filter)
                    .select("empId url username email phone role isActive isVerified facebook linkedin instagram twitter")
    ...
    static updateUserController = async (req,res) => {
        try {
            ...
            if (req.body.oldPassword) {
                const comparePassword = await bcrypt.compare(req.body.oldPassword,req.user.password || findDoc.password);
                if (!comparePassword) {
