...
class controller {
    ...
    static create = async (req,res) => {
        try {
            const {
                childSku,shortDescription,isDraft,material,metalColor,purity,weight,length,width,height,size,
                range,attributes,attTitle,settingType,attWeight,attName,number,cost,ratePerGram,costWeight,costName,
                amount,costDiscount,saveCost,costType,costDiscountType,metal,productSlug,discountDescription,title,
                quantity,rating,sales,expiresOn,availability
            } = req.body;

            let {taxValue,totalCost,subTotal,discountValue,discountType,savedAmount,taxAmount} = req.body;

            if (title) {
                const existingTitle = await ProductVariantModel.findOne({title: title});
                if (existingTitle) {
            ...
            if (childSku) {
                const existingSku = await ProductVariantModel.findOne({childSku: childSku});
                if (existingSku) {
                    return errorResponse({
                        res,
                        statusCode: 409,
            ...
            if (!proSlug) {
                return errorResponse({res,status: 404,error: new Error("Product is not found.")});
            }
            if (proSlug.hasVariant === false) {
                return errorResponse({res,status: 404,error: new Error("This product has no variant.")});
            }
            ...
            if (cost) {
                ...
                subTotal = cost.reduce((acc,item) => acc + item.totalCost,0) + price || 0;
            }
            ...
            if (!taxValue) {grandTotal = subTotal;}
            ...
            const doc = {
                files: uploadedImages.map((image) => ({urls: image.url})),taxAmount,
                product: proSlug._id,childSku,shortDescription,isDraft,material,metalColor,purity,weight,length,width,height,size,range,price,
                taxValue,attributes,attTitle,settingType,attWeight,attName,number,cost,ratePerGram,costWeight,costName,amount,
                costDiscount,saveCost,costType,costDiscountType,totalCost,savedAmount,discountType,quantity,rating,sales,availability,
                discountDescription,grandTotal,subTotal,discountValue,title,slug,createdBy: userId,expiresOn,metal,productSlug
            };
    ...
    static getVariant = async (req,res) => {
        try {
            ...
            if (productSlug) filter.productSlug = {$regex: new RegExp(`^${ productSlug }$`,'i')};
    ...
    static getStatusAndPrice = async (req,res) => {
        try {
            const {id} = req.params;
            const {product,metalColor,purity,size,attName,slug,productSlug} = req.query;
            ...
            if (productSlug) matchStage.productSlug = {$regex: new RegExp(`^${ productSlug }$`,'i')};
    ...
    static getByAdmin = async (req,res) => {
        try {
            const {id} = req.params;
            const {product,purity,metalColor,availability,search,size,slug,manager,attName,expiresOn,productSlug} = req.query;
            const matchStage = {};
    ...
    static getDetails = async (req,res) => {
        try {
            const {id} = req.params;
            const {attName,product,productSlug,metalColor,purity,size,slug,manager} = req.query;
            const matchStage = {};
    ...
    static update = async (req,res) => {
        try {
            const {id} = req.params;
            const {
                shortDescription,isDraft,material,metalColor,purity,weight,length,width,height,size,expiresOn,metal,
                range,price,attributes,attTitle,settingType,attWeight,attName,number,cost,ratePerGram,costWeight,costName,
                amount,costDiscount,saveCost,costType,discountDescription,title,quantity,rating,sales,availability
            } = req.body;

            let {taxValue,discountType,discountValue,savedAmount,totalCost,taxAmount} = req.body;
            ...
            } else if (existingVariant.discountValue && existingVariant.discountType) {
                ({newSubTotal: subTotal,savedAmount} = applyDiscount(existingVariant.discountValue,existingVariant.discountType));
            }
            ...
            const doc = {
                shortDescription,isDraft,material,metalColor,purity,weight,length,width,height,size,quantity,rating,sales,taxAmount,
                range,price,taxValue,attTitle,settingType,attWeight,attName,number,ratePerGram,costWeight,costName,slug,title,
                ...
                cost: cost ? [ ...existingVariant.cost,...cost ] : existingVariant.cost,availability,expiresOn,metal
            };
    ...
    static updateCost = async (req,res) => {
        try {
            ...
            if (existingVar.cost) {
                ...
                subTotal = existingVar.cost.reduce((acc,item) => acc + item.totalCost,0) + existingVar.price || 0;
            }
