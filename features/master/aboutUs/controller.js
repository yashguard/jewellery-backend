...
class controller {
    ...
    static update = async (req,res) => {
        try {
            ...
            if (!findDoc) {
                return errorResponse({
                    res,statusCode: 404,error: Error("Document not found.")
                });
            }
    ...
    static updateKey = async (req,res) => {
        try {
            ...
            const {scoreTitle,isProgress,symbol,number,link,mediaType} = req.body;
            ...
            if (findDoc.scores) {
                const attIndex = findDoc.scores.findIndex(scores => scores._id.toString() === keyId);
                if (attIndex !== -1) {
            ...
            if (findDoc.media) {
                const mediaIndex = findDoc.media.findIndex(media => media._id.toString() === keyId);
                if (mediaIndex !== -1) {
            ...
            if (!updated) {
                return errorResponse({res,statusCode: 404,error: Error('Attribute or Media not found')});
            }
    ...
    static remove = async (req,res) => {
        try {
            ...
            await findDoc.save();
            return successResponse({res,statusCode: 200,message: "Attribute or Media removed successfully."});
        } catch (error) {
    ...
    static delete = async (req,res) => {
        try {
            ...
            return successResponse({
                res,statusCode: 200,message: "About us deleted successfully."
            });
        } catch (error) {
            return errorResponse({
                res,
