...
class controller {
    ...
    static create = async (req,res) => {
        try {
            const {
                ...
                purity,weight,length,width,height,size,range,expiresOn,cost,costWeight,saveCost,costType,label,
            ...
            const doc = {
                files: uploadedImages.map((image) => ({urls: image.url})),taxValue,taxAmount,
    ...
    static getProduct = async (req,res) => {
        try {
            ...
            const {
                ...
                collections,category,availability,slug,subcategorySlug,categorySlug,featured,discount
            } = req.query;
            ...
            if (metal) {
                matchStage.attributes = {$elemMatch: {metal: {$regex: `^${ metal }$`,$options: "i"}}};
            }
    ...
    static getByAdmin = async (req,res) => {
        try {
            const {id} = req.params;
            const {search,availability,slug} = req.query;
